...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 4. SAFE-HOOK: enviar al WhatsApp real
    (async () => {
      try {
        const convSnap = await db.collection("conversations").doc(validation.conversationId).get();
        if (convSnap.exists) {
          const convData = convSnap.data();
          if (convData && convData.phone) {
            ...
            if (!sendResult.success) {
              log("error", "api/send-as-human", "Fallo al enviar por WhatsApp", { error: sendResult.error });
            }
          }
        }
      } catch (err) {
        log("error", "api/send-as-human", "Fallo al procesar envío de WhatsApp real", {
          error: String(err),
        });
      }
    })();
